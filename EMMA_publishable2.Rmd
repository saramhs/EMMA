...
dataFolder   <- file.path("/Users/mohan197/EMMA_Marcell/Directory/Data/") # Set path to the data and figure folder
figureFolder <- file.path("/Users/mohan197/EMMA_Marcell/Directory/Figures/")
...
season_lm <- residual2_combined_stf %>%
  ...
  do({
    ...
    # Tidy and glance the model
    tidy_model <- broom::tidy(model)
    glance_model <- broom::glance(model)
    
    # Print intermediate outputs for debugging
    print(paste("Season:", unique(.$season_stf)))
    print(tidy_model)
    print(glance_model)
    
    # Return a tibble with the model, tidy_model, and glance_model
    tibble(
      model = list(model),
      tidy_model = list(tidy_model),
      glance_model = list(glance_model)
    )
  }) %>%
  mutate(
    intercept = sapply(tidy_model, function(x) {
      print("Intercept calculation")
      print(x)
      x$estimate[1]
    }),
    slope = sapply(tidy_model, function(x) {
      print("Slope calculation")
      print(x)
      x$estimate[2]
    }),
    r_squared = sapply(glance_model, function(x) {
      print("R-squared calculation")
      print(x)
      x$r.squared
    }),
    p_value = sapply(glance_model, function(x) {
...
#  df<-dplyr::select(df,date,DoY,name,cl, so4, ca, k , mg, na, al, fe, mn,si,sr,srp, tn, tn, toc)
#  return(df)
#}
...
diff_days_func<-function(df_EM,df_stf){
  df_EM<-df_EM%>%mutate(diff_days=abs(DoY-(df_stf$DoY)))
  return(df_EM)
}
...
#for all stf
stf_predicted_lkback <- as.data.frame(matrix(NA, nrow = nrow(stream_ch_df), ncol = 4))
colnames(stf_predicted_lkback) <- c("ca", "mg", "mn", "si")#,"date","name","DoY","season")
EMs_opt_stf_list<-list()
EMs_ch_opt_stf_list<-list()
cont_opt_stf_list<-list()
dist_opt_stf_list<-list()
  ...
  for (em2 in seq_len(nrow(EM2_proj))) {
    ...
    for (em3 in seq_len(nrow(EM3_proj))) {
      ...
      if (dett == 0) next 
               ...
               contributions_lkback <- calculate_contributions(stream_obs, EM1, EM2, EM3, dett)
            
              if (any(contributions_lkback[1:3] < 0)){
             
                contributions_lkback <- adjust_contributions(contributions_lkback)
                cont_comb<-contributions_lkback #num turns into list feature
              }

        for (col in 1:4) {
          stf_predicted_lkback[stf, col] <- cont_comb[1] * EM1_ch[col] + cont_comb[2] * EM2_ch[col] + cont_comb[3] * EM3_ch[col]
        }


        distance_comb <- distance(stf_predicted_lkback[stf,],stream_obs_ch) 
        dist_comb_list[[length(dist_comb_list)+1]]<-distance_comb
        cont_comb_list[[length(cont_comb_list)+1]]<-cont_comb
...
plot_data <-plot_data(stf_predicted_model2,stream_ch_df,col)
  ...
  #ca mg mn si
   ...
   #   x = "Ca (observed)",
    #  y = "Ca (predicted)",
      title ="Mn", #paste0(names(stf_predicted)[col]), 
